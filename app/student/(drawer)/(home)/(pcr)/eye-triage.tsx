...
const EyeTriage = () => {
	...
	const { patientId, recordId, fullName }: any = useGlobalSearchParams();
					...
					try {
						...
						if (fetchedEyeTriage) {
							...
							Object.keys(fetchedEyeTriage).forEach((key) => {
								setValue(
									key as keyof EyeTriage,
									fetchedEyeTriage[key]
								);
							});
						} else {
							return;
	...
	const onSubmit = async (data: EyeTriage) => {
		...
		try {
			...
			Alert.alert("Success", "Eye triage has been completed!", [
				{ text: "Okay" },
			]);
			router.back();
		} catch (error) {
		...
		>
			...
			{patient.records[0]?.eyeTriage?.isComplete && (
				...
				>
					...
					<SizableText size={"$1"} color={"$green3"}>
						{moment(
							patient.records[0]?.eyeTriage?.createdAt
			...
			>
				<View padding="$5">
					...
					/>
					<SizableText mt="$4">
						Was the patient experiencing any flashes of light or
						floaters in their vision?
						<SizableText color={"red"}>*</SizableText>
					</SizableText>
					<Controller
						control={control}
					...
					/>
					{errors.hasFlashesOrFloaters && (
						<SizableText size={"$2"} color="red">
							{errors.hasFlashesOrFloaters.message}
						</SizableText>
					)}
					...
					{showFlashesQuestion && (
						<Animated.View entering={FadeIn} exiting={FadeOut}>
							<TextInput
								...
								label="How long did the patient notice the flashes?"
								required
							/>
							<SizableText mt="$4">
								Have they increased in number?
							</SizableText>
							<Controller
								control={control}
								name="flashes"
							...
							{errors.flashes && (
								<SizableText size={"$2"} color="red">
									{errors.flashes.message}
								</SizableText>
							)}
							<TextInput
								name="symptomBegins"
								control={control}
								label="When did the symptoms begin, and have they worsened over time?"
								placeholder={"Specify"}
								required
							/>
						</Animated.View>
